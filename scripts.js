...
async function loadMadeWithLove() {
  const mount = document.getElementById('made-with-love-mount');
  if (!mount?.dataset.componentSrc) return;

  try {
    const res = await fetch(mount.dataset.componentSrc);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const html = await res.text();
    const doc = new DOMParser().parseFromString(html, 'text/html');
    const sourceLink = doc.getElementById('share-love-btn');
    if (!sourceLink) throw new Error('Missing #share-love-btn');

    const nav = document.createElement('nav');
    nav.className = 'made-love-nav';
    nav.setAttribute('aria-label', 'Share this site');
    const span = document.createElement('span');
    const link = document.createElement('a');
    link.id = 'share-love-btn';
    link.className = sourceLink.className || 'tt-bottom-left';
    link.href = '#';
    link.role = sourceLink.getAttribute('role') || 'button';
    link.setAttribute('aria-label', sourceLink.getAttribute('aria-label') || 'Copy page link to share');
    link.dataset.tooltip = sourceLink.dataset.tooltip || 'Share with love';
    link.textContent = sourceLink.textContent;
    span.appendChild(link);
    nav.appendChild(span);
    mount.replaceChildren(nav);
    initMadeWithLoveShare(link);
  } catch (e) {
    console.warn('Made with love component not loaded.', e);
    mount.innerHTML = '<span class="made-love-fallback">[made with 🩶]</span>';
  }
}
...
function initMadeWithLoveShare(link) {
  ...
  link.addEventListener('click', async (event) => {
    ...
    const text = 'Check out AI Decoded: ' + pageUrl;
