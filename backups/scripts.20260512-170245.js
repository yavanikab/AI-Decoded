...
function getTermSearchText(term) {
  if (!term) return '';
...
function termMatchesQuery(term, q) {
  return getTermSearchText(term).includes(q);
}
...
function renderHomeSearchResults(q) {
  ...
  const matches = CATEGORIES
    ...
    .filter(term => termMatchesQuery(term, q));
  ...
  resultsEl.innerHTML = `
    <div class="search-results-head">${matches.length} matching ${matches.length === 1 ? 'term' : 'terms'}</div>
...
function runSearch(rawQuery) {
  ...
  document.querySelectorAll('.term-row').forEach(r => {
    ...
    r.style.display = (term ? termMatchesQuery(term, q) : name.includes(q)) ? '' : 'none';
  });
